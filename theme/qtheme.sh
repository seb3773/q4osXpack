...
ktext="<font style='color:#ac6009'><strong>⏺ q4osXpack Theme ⏺</strong></font><br><br>"
kdtitle="q4osXpack"
kdcaption="qtheme"
qtheme1() {
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Fthememode=$(kdialog --icon "$kdicon" --title "$kdtitle" --caption "$kdcaption" --geometry $(centerk 400 350) --menu "$kdtext" "Classic windows 10 theme" "Classic windows 10 theme" "Dark windows 10 theme" "Dark windows 10 theme" "Mac OSX theme" "Mac OSX theme" --default "Classic windows 10 theme")
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
if [ $? -eq 1 ];then
echo "Canceled."
...
Fthemecolor=$(kdialog --icon "$kdicon" --title "$kdtitle" --caption "$kdcaption" --geometry $(centerk 400 350) --menu "$kdtext" "Theme default color accent" "Theme default color accent" "Custom color accent" "Custom color accent" --default "Theme default color accent")
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
if [ $? -eq 1 ];then
echo "Canceled."
else
...
"
kdialog --icon "$kdicon" --title "$kdtitle" --caption "$kdcaption" --geometry $(centerk 450 350) --warningcontinuecancel "$kdtext"
}
...
═══════════════════════════<br>
<em>►$thememode selected.</em><br>"
if [ ! "$thememode" = "Mac OSX theme" ]; then
kdtext="$kdtext<em>►$themecolor</em><br>";fi
kdtext="$kdtext═══════════════════════════<br><br>
<font style='color:#828282'>►</font> Proceed with these settings ?<br>
<font style='color:#828282'><em>(or hit cancel to return to theme choice)</em></font>
"
...
oksu=$(sudo -n echo 1 2>&1 | grep 1);if [[ ! $oksu -eq 1 ]];then kdialog --title "q4osXpack » qtheme " --icon "$kdicon" --msgbox "Can't continue without root authentification.";exit;fi
...
konsole --nomenubar --nohist --notabbar --noframe --noscrollbar --vt_sz 88x26 -e common/apt_update.sh "$dcopRef" "theme" -- &
kid=konsole-$!
...
echo -e "   ░▒▓█ Retrieve installed packages list..."
echo
cd common
sudo ./pklist
cd ..
...
dcop "$dcopRef" setLabel "Installing 7z..."
echo -e "    ░▒▓█ Installing 7z..."
echo -e "${YELLOW}"
...
dcop "$dcopRef" setLabel "Install plymouth..."
...
dcop "$dcopRef" setLabel "Install plymouth-themes..."
...
dcop "$dcopRef" setLabel "Install Q4Win10 plymouth theme..."
...
dcop "$dcopRef" setLabel "Install grub theme..."
...
dcop "$dcopRef" setLabel "Install shutdown image..."
...
dcop "$dcopRef" setLabel "Configuring boot settings..."
...
dcop "$dcopRef" setLabel "Copying files..."
if [[ ! $osx -eq 1 ]]; then
...
dcop "$dcopRef" setLabel "Install desktop splash screen..."
...
rota  "$dcopRef" "Configuring start menu..."
...
dcop "$dcopRef" setLabel "Installing kooldock..."
...
dcop "$dcopRef" setLabel "Installing lightpad dependencies..."
...
dcop "$dcopRef" setLabel "Installing gtk2-engines-murrine.."
...
dcop "$dcopRef" setLabel "Installing gtk2-engines-pixbuf.."
...
dcop "$dcopRef" setLabel "Configuring windows style..."
echo -e "  ░▒▓█ installing qtcurve-trinity..."
...
dcop "$dcopRef" setLabel "Configuring windows decoration & windows management..."
echo
echo -e "  ░▒▓█ installing Dekorator for trinity..."
...
dcop "$dcopRef" setLabel "Installing imagemagick..."
...
qprogress "$script" 35
echo -e "  ░▒▓█ Generating windows decorations..."
dcop "$dcopRef" setLabel "Generating windows decorations..."
...
echo
echo -e "  ░▒▓█ configuring style..."
dcop "$dcopRef" setLabel "Configuring style..."
...
rota  "$dcopRef" "Configuring style..."
...
echo -e "  ░▒▓█ enabling compton-tde compositing..."
dcop "$dcopRef" setLabel "Enabling compton-tde compositing..."
...
echo -e "  ░▒▓█ configuring GTK2/GTK3/GTK4 styles..."
dcop "$dcopRef" setLabel "configuring GTK2/GTK3/GTK4 styles..."
...
echo -e "  ░▒▓█ configuring xcompmgr..."
dcop "$dcopRef" setLabel "configuring xcompmgr..."
sudo tar -xzf theme/xcompmgrrc.tar.gz -C $USER_HOME/
sudo tar -xzf theme/xcompmgrrc.tar.gz -C /root/

qprogress "$script" 52

echo -e "  ░▒▓█ configuring compton-tde..."
dcop "$dcopRef" setLabel "configuring compton-tde..."
if [[ $dark -eq 1 ]]; then
...
echo -e "  ░▒▓█ disable screensaver & lock after suspend..."
dcop "$dcopRef" setLabel "disabling screensaver..."
...
echo -e "  ░▒▓█ Configuring Konqueror ui..."
dcop "$dcopRef" setLabel "Configuring Konqueror ui..."
...
rota  "$dcopRef" "Configuring Konqueror ui..."
...
echo -e "  ░▒▓█ Configuring Dolphin ui..."
dcop "$dcopRef" setLabel "Configuring Dolphin ui..."
...
rota "$dcopRef" "Configuring Dolphin ui..."
if [[ $osx -eq 1 ]]; then
...
#~~~~~~~~~~~~~~~~~~~~~ end if dolphin
echo -e "  ░▒▓█ Adjusting folders icons"
dcop "$dcopRef" setLabel "Adjusting folders icons"
...
Icon=folder-videos
EOF
rota "$dcopRef" "Adjusting folders icons"
echo
printf '\e[A\e[K'
sep
echo
...
dcop "$dcopRef" setLabel "Disabling start indicator"
...
dcop "$dcopRef" setLabel "Configuring pointers..."
#pointer size
#32 /48 /64 /
...
dcop "$dcopRef" setLabel "Applying color scheme & wallpaper..."
if [[ ! $osx -eq 1 ]]; then
wallpn=$(( $RANDOM % 21 + 1 ))
...
rota "$dcopRef" "Applying color scheme & wallpaper..."
...
dcop "$dcopRef" setLabel "Configuring login style..."
echo
echo -e "  ░▒▓█ installing tdmtheme-trinity..."
...
echo -e "  ░▒▓█ configuring wallpaper for login & ksplash"
echo "       (please be patient this could take some time...)"
dcop "$dcopRef" setLabel "configuring wallpaper for login & ksplash..."
...
dcop "$dcopRef" setLabel "configuring taskbar..."
	if [[ $dark -eq 1 ]]; then
...
if [[ ! $osx -eq 1 ]]; then
if grep -q "weather_panelappletrc" "$TDEHOME/share/config/kicker_menubarpanelrc" || grep -q "weather_panelappletrc" "$TDEHOME/share/config/kickerrc"; then
cleankickerrc
cat theme/kickpart2 >> $TDEHOME/share/config/kickerrc
kwriteconfig --file $TDEHOME/share/config/kickerrc --group General --key Applets2 "KMenuButton_1,ExtensionButton_1,WindowListButton_1,Applet_4,Applet_1,Applet_2,Applet_7,Applet_3,Applet_6,Applet_5"
kwriteconfig --file $TDEHOME/share/config/weather_panelappletrc --group "General Options" --key "smallview_mode" 3
if [[ $dark -eq 1 ]]; then
kwriteconfig --file $TDEHOME/share/config/weather_panelappletrc --group "General Options" --key "textColor" "255,255,255"
else
kwriteconfig --file $TDEHOME/share/config/weather_panelappletrc --group "General Options" --key "textColor" "0,0,0"
fi
else
cleankickerrc
...
sudo chown $USER: $TDEHOME/share/config/kickerrc
sudo chmod 644 $TDEHOME/share/config/kickerrc
...
if grep -q "weather_panelappletrc" "$TDEHOME/share/config/kicker_menubarpanelrc" || grep -q "weather_panelappletrc" "$TDEHOME/share/config/kickerrc"; then
cleankickerrc
tar -xzf theme/kicker_menubarpanelrc2.tar.gz -C $TDEHOME/share/config/
kwriteconfig --file $TDEHOME/share/config/weather_panelappletrc --group "General Options" --key "smallview_mode" 2
kwriteconfig --file $TDEHOME/share/config/weather_panelappletrc --group "General Options" --key "textColor" "0,0,0"
else
cleankickerrc
...
echo -e "  ░▒▓█ installing showdeskten kicker applet"
dcop "$dcopRef" setLabel "installing showdeskten kicker applet"
...
echo -e "  ░▒▓█ installing actioncenter kicker applet"
dcop "$dcopRef" setLabel "installing actioncenter kicker applet"
...
dcop "$dcopRef" setLabel "installing xdotool"
...
dcop "$dcopRef" setLabel "installing xsel"
...
echo -e "  ░▒▓█ installing about this mac..."
dcop "$dcopRef" setLabel "installing about this mac..."
...
dcop "$dcopRef" setLabel "Configuring systray clock..."
...
dcop "$dcopRef" setLabel "Configuring fonts..."
...
kwriteconfig --file $TDEHOME/share/config/kdeglobals --group General --key fixed "SF Mono,10,-1,5,50,0,0,0,0,0"
...
kwriteconfig --file $TDEHOME/share/config/konsolerc --group "Desktop Entry" --key defaultfont "SF Mono,10,-1,5,50,0,0,0,0,0"
qprogress "$script" 90
kwriteconfig --file $TDEHOME/share/config/konsolerc --group "Desktop Entry" --key TabColor "0,0,0"
...
kwriteconfig --file $TDEHOME/share/config/krusaderrc --group UserActions --key "Fixed Font" "SF Mono,10,-1,5,50,0,0,0,0,0"
#kwriteconfig --file $TDEHOME/share/config/krusaderrc --group UserActions --key "Fixed Font" "Cascadia Code,11,-1,5,50,0,0,0,0,0"
...
kwriteconfig --file $USER_HOME/.tderc --group General --key fixed "SF Mono,10,-1,5,50,0,0,0,0,0"
...
#root
#sudo kwriteconfig --file /root/.trinity/share/config/kdeglobals --group General --key fixed "Droid Sans Mono,9,-1,5,50,0,0,0,0,0"
sudo kwriteconfig --file /root/.trinity/share/config/kdeglobals --group General --key fixed "SF Mono,10,-1,5,50,0,0,0,0,0"
#sudo kwriteconfig --file /root/.trinity/share/config/kdeglobals --group General --key fixed "Cascadia Code,11,-1,5,50,0,0,0,0,0"
...
sudo kwriteconfig --file /root/.trinity/share/config/konsolerc --group "Desktop Entry" --key defaultfont "SF Mono,10,-1,5,50,0,0,0,0,0"
#kwriteconfig --file /root/.trinity/share/config/konsolerc --group "Desktop Entry" --key defaultfont "Cascadia Code,10,-1,5,50,0,0,0,0,0"
qprogress "$script" 91
sudo kwriteconfig --file /root/.trinity/share/config/konsolerc --group "Desktop Entry" --key TabColor "0,0,0"
...
sudo kwriteconfig --file /root/.trinity/share/config/krusaderrc --group UserActions --key "Fixed Font" "SF Mono,10,-1,5,50,0,0,0,0,0"
#kwriteconfig --file /root/.trinity/share/config/krusaderrc --group UserActions --key "Fixed Font" "Cascadia Code,11,-1,5,50,0,0,0,0,0"
...
sudo kwriteconfig --file $USER_HOME/.tderc --group General --key fixed "SF Mono,10,-1,5,50,0,0,0,0,0"
...
dcop "$dcopRef" setLabel "Configuring icons..."
...
dcop "$dcopRef" setLabel "Configuring « usb fast removal »..."
...
dcop "$dcopRef" setLabel "Installing admin tools..."
...
dcop "$dcopRef" setLabel "Installing themepack/.deskthemepack installer..."
...
dcop "$dcopRef" setLabel "Configuring global shortcuts & default apps integration..."
#enable kcontrol
...
echo -e "  ░▒▓█ strawberry installed - Applying strawberry theme..."
dcop "$dcopRef" setLabel "strawberry installed - Applying strawberry theme..."
...
dcop "$dcopRef" setLabel "Cleaning temp files..."
